...
const collegeController = {
    ...
    createCollege: errorMiddleware.catchAsync(async (req, res) => {
        ...
        // Track college creation in analytics
        await createInitialCollegeAnalytics(college);
    ...
    updateCollege: errorMiddleware.catchAsync(async (req, res) => {
        ...
        // Track college update in analytics
        await updateCollegeAnalytics(college._id, 'college_updates', {
    ...
    deleteCollege: errorMiddleware.catchAsync(async (req, res) => {
        ...
        // Track college deletion in analytics
        await updateCollegeAnalytics(college._id, 'college_deletions', {
    ...
    inviteToCollege: errorMiddleware.catchAsync(async (req, res) => {
        ...
        await updateCollegeAnalytics(collegeId, 'invitations_sent', {
            role: role || 'student',
    ...
    acceptInvitation: errorMiddleware.catchAsync(async (req, res) => {
        ...
        // Track invitation acceptance in analytics
        await updateCollegeAnalytics(invitation.collegeId, 'invitations_accepted', {
            role: invitation.role
        });
    ...
    rejectInvitation: errorMiddleware.catchAsync(async (req, res) => {
        ...
        // Track invitation rejection in analytics
        await updateCollegeAnalytics(invitation.collegeId, 'invitations_rejected');
    ...
    updateCollegeSettings: errorMiddleware.catchAsync(async (req, res) => {
        ...
        await updateCollegeAnalytics(collegeId, 'settings_updates', {
            configUpdates: config ? Object.keys(config).length : 0,
            settingsUpdates: settings ? Object.keys(settings).length : 0
        });
    ...
    upgradeSubscription: errorMiddleware.catchAsync(async (req, res) => {
        ...
        // Track subscription upgrade in analytics
        await updateCollegeAnalytics(collegeId, 'subscription_upgrade', {
            fromPlan: college.subscription.history?.[college.subscription.history.length - 1]?.plan || 'free',
            toPlan: plan
        });
    ...
    cancelSubscription: errorMiddleware.catchAsync(async (req, res) => {
        ...
        await updateCollegeAnalytics(collegeId, 'subscription_cancelled', {
...
async function createInitialCollegeAnalytics(college) {
    try {
        const now = new Date();
        ...
        const analytics = new Analytics({
            ...
            timestamp: today,
            users: {
                total: 1, // Starting with the admin who created it
                active: 1,
                new: 1,
                byRole: new Map([['college_admin', 1]])
            },
            status: {
                isProcessed: true
            }
        ...
        console.error('Error creating initial college analytics:', error);
    }
}

async function updateCollegeAnalytics(collegeId, metric, details = {}) {
    try {
        const now = new Date();
        const today = new Date(now.getFullYear(), now.getMonth(), now.getDate());
        ...
        switch (metric) {
            case 'invitations_sent':
                analytics.users.invitationsSent = (analytics.users.invitationsSent || 0) + 1;
                break;
            case 'invitations_accepted':
                analytics.users.invitationsAccepted = (analytics.users.invitationsAccepted || 0) + 1;
                break;
            case 'invitations_rejected':
                analytics.users.invitationsRejected = (analytics.users.invitationsRejected || 0) + 1;
                break;
            case 'college_updates':
                analytics.users.collegeUpdates = (analytics.users.collegeUpdates || 0) + 1;
                break;
            case 'settings_updates':
                analytics.users.settingsUpdates = (analytics.users.settingsUpdates || 0) + 1;
                break;
            case 'subscription_upgrade':
                analytics.business.revenue = (analytics.business.revenue || 0) + 100; // Example revenue
                break;
            case 'subscription_cancelled':
                analytics.business.churn = (analytics.business.churn || 0) + 1;
                break;
            case 'college_deletions':
                analytics.users.collegeDeletions = (analytics.users.collegeDeletions || 0) + 1;
                break;
        }

        await analytics.save();
    } catch (error) {
        console.error('Error updating college analytics:', error);
    }
}
